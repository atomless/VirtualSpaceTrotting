...
Virtual Globetrotting also provides the instrumentation model this project should mirror: its live page markup configures `window.BOOMR_config`, sets a public `window.BOOMR_API_key`, and loads Boomerang from `https://c.go-mpulse.net/boomerang/<mpulse-api-key>`. This project must use the same origin-side Boomerang pattern, but with its own Akamai mPulse app key.

References:

- [Virtual Globetrotting](https://virtualglobetrotting.com/)
...
Use the Virtual Globetrotting pattern as the local model:
...
Do not reuse Virtual Globetrotting's key. Replace the placeholder with this project's generated mPulse API key and prefer the exact loader snippet generated by Akamai for the app.
